...
function errHighlight(el) {
   el.style['border'] = '1px solid red';
}
function errUnHighlight(el) {  
   el.style['border'] = '';
}


function shrinkSuccess() {
   if ($('#success'))
...
$(function() {
   AEHit();
   setTimeout(shrinkSuccess, 4000);
   
   if ('#loggedin') {
      $.get('show_logged_in',{}, function(data) {
	 if (data) {
	    $('#loggedin').html(data);
	 }
      });
